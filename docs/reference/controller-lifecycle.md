...
Status: **experimental library API; no privileged daemon or protocol yet**
...
The lifecycle object and failure-injection tests are implemented. Tests use a
controlled `pfctl` substitute and never touch the host firewall. There is no
privileged daemon entry point, Unix-domain socket, peer authentication, protocol
schema, supervisor integration, `pledge`, or `unveil` policy yet. Real PF
activation therefore remains disabled pending a disposable OpenBSD lab path and
